...
namespace CrawlerSpace
{
	...
	private:
		...
		static void work(string input_folder, string output_folder, queue<string>& p_q,
			map<string, bool>& passed_page, atomic<size_t>& running_workers_number);
		double work_time;
		static atomic<size_t> visited_counts;
	public:
		Crawler() : work_time(0) {}
		void RunCrawler(size_t th_counts, string start_source, string input_folder, string output_folder);//return time of crawler work in seconds
		void RunCrawler(string start_source, string input_folder, string output_folder) { this->RunCrawler(1, 
			start_source, input_folder, output_folder); }
